...
#include <autolabor_pro1_driver/UpdateOdom.h>
...
private:
  ...
  bool updateOdom(autolabor_pro1_driver::UpdateOdom::Request  &req,
				autolabor_pro1_driver::UpdateOdom::Response &res);
...
private:
  ...
  ros::ServiceServer update_odom_server_;
  struct OdomIncrement
  {
  	double x,y,theta;
  };
  std::unique_ptr<OdomIncrement> odom_increment_; 
};
...
bool ChassisDriver::updateOdom(autolabor_pro1_driver::UpdateOdom::Request  &req,
							   autolabor_pro1_driver::UpdateOdom::Response &res)
{
  if(req.password != 326326)
  {
  	res.success = false;
  	return true;
  }
  ...
  tf_listener.waitForTransform("/map", "/odom", ros::Time(0), ros::Duration(1.0));
  tf_listener.lookupTransform("/map", "/odom", ros::Time(0), tf_map2odom);
  
  odom_increment_.reset(new OdomIncrement);
  
  auto quat = tf_map2odom.getRotation();
  double roll, pitch, yaw;
  tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);
  
  auto origin = tf_map2odom.getOrigin();
  odom_increment_->x = origin.getX();
  odom_increment_->y = origin.getY(); 
  odom_increment_->theta = yaw;
  
  ROS_INFO("delta_x:%f  delta_y:%f  delta_th:%f",odom_increment_->x,odom_increment_->y,odom_increment_->theta);
...
void ChassisDriver::run(){
  ...
  private_node.param<bool>("is_tf_broadcast", is_send_tf_transform_, false);
  ...
  if (init()){
    ...
    update_odom_server_ = node.advertiseService("update_odom",&ChassisDriver::updateOdom,this);
    ros::spin();
    return;
  }
}
